...
def generate_text(prompt: str, max_new_tokens: int = 220) -> str:
    txt, err = _run_models(TEXT_MODELS, [{"role": "user", "content": prompt }], max_tokens=max_new_tokens, temprature=0.4)
    if not txt:
        raise Exception(err)
    return txt
...
def generate_exact_sentence(prompt: str, n_words: int, max_new_tokens: int, tries: int = 6) -> str:
    ...
    for _ in range(tries):
        last = generate_exact_sentence(prompt, max_new_tokens=max_new_tokens)
...
def get_basic_caption(image_path: str) -> str:
    ...
    msgs = [{
        "role": "user",
        "content" : [
            {"type": "text", "text": "Write complete sentence describing this image"},
